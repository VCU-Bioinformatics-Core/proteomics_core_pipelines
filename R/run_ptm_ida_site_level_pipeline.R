...
#' )
#' }
run_ptm_ida_site_level_pipeline <- function(
  run_id,
  samplesheet_file,
  ptm_matrix_file,
  protein_matrix_file,
  ...
  full_ptm_peptide_levels <- full_ptm_peptide_levels %>%
    filter(grepl(ptm_marker, PTM.ModificationTitle))
  n_peptides_ptm <- nrow(full_ptm_peptide_levels)
  log_peptide_count(full_ptm_peptide_levels, "Peptides - only PTM")

  # remove peptides with a gene == NA
  full_ptm_peptide_levels <- full_ptm_peptide_levels %>% filter(!is.na(PG.Genes))

  # create a new collapse ID
  full_ptm_peptide_levels$Gene.PTM.Site <- sub("^[^_]+",
					       full_ptm_peptide_levels$PG.Genes,
					       full_ptm_peptide_levels$PTM.CollapseKey
					       )
  full_ptm_peptide_levels$Gene.PTM.Site <- paste0(full_ptm_peptide_levels$Gene.PTM.Site, "_", full_ptm_peptide_levels$PTM.FlankingRegion)

  # drop duplicates based on Gene.PTM.Site
  full_ptm_peptide_levels <- full_ptm_peptide_levels[!duplicated(full_ptm_peptide_levels$Gene.PTM.Site, fromLast = TRUE), ]

  # adding back the UniProId
  full_ptm_peptide_levels <- full_ptm_peptide_levels %>% mutate(PG.UniProtIds=PG.ProteinAccessions)

  # set rownames to Gene.PTM.Site
  rownames(full_ptm_peptide_levels) <- full_ptm_peptide_levels$Gene.PTM.Site

  # extracting numerical columns only and log2 transforming the data
  non_numeric_cols <- c("PG.ProteinAccessions", "PG.UniProtIds", "PG.Genes", "PTM.CollapseKey", "PTM.Multiplicity",
			    "PTM.ModificationTitle", "PTM.SiteAA", "PTM.SiteLocation", "PTM.FlankingRegion", "Gene.PTM.Site")
  ...
  for (i in seq_along(comparisons)) {
    ...
    curr_result <- run_single_ptm_ida_site_level_comparison(
